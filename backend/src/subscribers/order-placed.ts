...
export default async function userInviteHandler({
	...
	const orderModeulService: IOrderModuleService = container.resolve(
		Modules.ORDER
	);
	const order: OrderDTO = await orderModeulService.retrieveOrder(data.id, {
		relations: ["items", "summary", "shipping_address"],
	});

	const shippingAdress: OrderAddressDTO = await (
		orderModeulService as any
  ...
  <p><strong>Total:</strong> ${
	...
	} ${order.currency_code}</p>

  <h2>Shipping Address</h2>
  <p><strong>Name:</strong> ${shippingAdress.first_name} ${
		shippingAdress.last_name
	}</p>
  <p><strong>Address:</strong> ${shippingAdress.address_1}</p>
  <p><strong>City:</strong> ${shippingAdress.city}, ${
		shippingAdress.province
	} ${shippingAdress.postal_code}</p>
  ...
  <p>You can view your order details <a href="${backendUrl}/admin/orders/${
		order.id
	}">here</a>.</p>

  <p>Sincerely,<br>Mary Moda Exclusiva</p>
`;
